...
class CommandProcessorTests {
	...
	void runWhenNoArgumentThrowsException() {
		CommandProcessor processor = new CommandProcessor(Collections.singletonList(Mockito.mock(Command.class)));
	...
	void runWhenUnknownCommandThrowsException() {
		Command fooCommand = Mockito.mock(Command.class);
		...
		DefaultApplicationArguments args = new DefaultApplicationArguments("bar", "go");
	...
	void runDelegatesToCommand() throws Exception {
		Command fooCommand = Mockito.mock(Command.class);
		given(fooCommand.getName()).willReturn("foo");
		Command barCommand = Mockito.mock(Command.class);
